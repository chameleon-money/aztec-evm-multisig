...
async function main() {
  ...
  logger.info(
    `📊 Initial threshold: ${initialThreshold}/${initialSignerCount}`,
  );

  const { wormholeAddress } = await registerWormholeContracts(
    nodeClient,
    wallet,
  );
  ...
  const multisigContract = await deployTx.deployed({
