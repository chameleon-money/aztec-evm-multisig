...
export async function createAccountFromEnv(index: number, wallet: TestWallet): Promise<AccountManager> {
    ...
    const schnorrAccount = await wallet.createSchnorrAccount(secretKey, salt, signingKey);
...
export async function getAccountFromEnv(index: number, wallet: TestWallet): Promise<AccountManager> {
