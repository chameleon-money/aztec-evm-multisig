...
export async function createAccountFromEnv(
  ...
  const secretEnv = process.env[`SECRET_${index}`];
  const signingKeyEnv = process.env[`SIGNING_KEY_${index}`];
  const saltEnv = process.env[`SALT_${index}`];

  if (!secretEnv) {
    throw new Error(
      "SECRET environment variable is required. Please set it in your .env file.",
    );
  }

  if (!signingKeyEnv) {
    throw new Error(
      "SIGNING_KEY environment variable is required. Please set it in your .env file.",
    );
  }

  if (!saltEnv) {
    throw new Error(
      "SALT environment variable is required. Please set it in your .env file.",
    );
  }
  ...
  logger.info(
    "🏗️  Creating Schnorr account instance with environment values...",
  );
  ...
  try {
    ...
    if (isRegistered) {
      ...
      logger.info(
        "ℹ️  Account is not yet registered. You may need to deploy it first.",
      );
    }
  } catch (error) {
...
export async function getAccountFromEnv(
  ...
  return await createAccountFromEnv(index, wallet);
}
