...
import { getTimeouts, signer1, signer2, signer3, sponsoredFPC, wallet } from "./config.js";
...
async function main() {
    ...
    if (!contractAddress) {
        logger.error("Please set MULTISIG_CONTRACT_ADDRESS environment variable with your deployed contract address");
        return;
    }
    ...
    const multisigInstance = await nodeClient.getContract(multisigContractAddress);

    if (!multisigInstance) {
        throw new Error(`MultiSig contract instance not found at address ${multisigContractAddress}`);
    }

    await wallet.registerContract({ instance: multisigInstance, artifact: MultiSigContract.artifact });
    ...
    const tokenAddress = EthAddress.fromString('0x4a9088e41c625d13200a12a5952c0f9dbca9abfc');
    const receipientAddress = EthAddress.fromString('0x7e1b5dbb3cddc33b22b70d1890a5ba11a35d8921');
    const amount = parseEther("50"); // 1 ether in wei (as bigint)

    const nextProposalId = await multisigContract.methods.get_next_proposal_id().simulate({ from: signer1.address });

    console.log(nextProposalId);
    const proposeTx = await multisigContract.methods.propose_execute_transaction(
        tokenAddress,
        receipientAddress,
        amount
    ).send({
    ...
    const sign1Tx = await multisigContract.methods.sign_proposal(nextProposalId).send({
        from: signer1.address,
        fee: { paymentMethod: sponsoredPaymentMethod }
    }).wait({ timeout: timeouts.txTimeout });
    ...
    const sign2Tx = await multisigContract.methods.sign_proposal(nextProposalId).send({
    ...
    const sign3Tx = await multisigContract.methods.sign_proposal(nextProposalId).send({
    ...
    const executeTx = await multisigContract.methods.execute_proposal(nextProposalId).send({
        from: signer1.address,
        fee: { paymentMethod: sponsoredPaymentMethod }
    }).wait({ timeout: timeouts.txTimeout });
